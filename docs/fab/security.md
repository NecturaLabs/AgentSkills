...
The Rust build, tests and release matrix run on GitHub-hosted runners only, alongside the rest of
AgentSkills' CI. That CI triggers on `pull_request`, so a self-hosted runner must never serve any
of its jobs: for `pull_request` events GitHub runs the workflow file from the pull request itself,
and no condition written in a workflow can keep a fork's code off such a runner.
