...
rest. Exit codes and error codes: `plugin/skills/necturalabs-fab/references/errors.md`.

| Symptom | Cause | Fix |
|---|---|---|
| `FAB_PROVIDER_NOT_INSTALLED` | `fabcli` not on `PATH`. | Install it (`docs/installation.md`) or set `fabcli.path`. |
...
| Account sign-in window never appears (Linux) | WebKitGTK missing, or no graphical session. | Install the libraries in `docs/installation.md`. Reads sign in through the browser and do not need it. |
