# necturalabs-fab installer and uninstaller for Windows (PowerShell 5.1 or later).
#
...
#
# Mirrors scripts/install.sh: installs FabCLI (pinned, checksum-verified) and necturalabs-fab into one
# bin directory on the user PATH, installs the agent skill as a plugin through the Claude Code and
# Codex plugin marketplaces, and offers to sign in. Everything it creates is recorded in a manifest;
# -Uninstall removes exactly that, plus the caches those harnesses leave behind, and nothing else.
...
param(
  ...
  [string]$Source = '',
  [switch]$NoPlugin = ($env:NECTURALABS_FAB_NO_PLUGIN -eq '1'),
...
function Install-ClaudePlugin {
  $ErrorActionPreference = 'Continue'
  if (-not (Have 'claude')) { Say 'skill: Claude Code not found; skipped'; return }
  $src = if ($state.Source) { $state.Source } else { "$Repo#$($state.Version)" }
  # A marketplace we added earlier points at the release installed then; move it to this one.
  if ((Test-ClaudeMarketplace) -and (Recorded 'claude_marketplace') -eq 'added' -and (Recorded 'claude_marketplace_source') -ne $src) {
    Invoke-Checked 'removing the old Claude Code marketplace' { claude plugin marketplace remove $Marketplace }
  }
  if (Test-ClaudeMarketplace) {
    if (-not (Recorded 'claude_marketplace')) { Record 'claude_marketplace' 'preexisting' }
    if (-not (Invoke-Quiet { claude plugin marketplace update $Marketplace })) { Warn 'could not refresh the Claude Code marketplace' }
  } else {
    Invoke-Checked "adding the Claude Code marketplace from $src" { claude plugin marketplace add $src }
    Record 'claude_marketplace' 'added'
    Record 'claude_marketplace_source' $src
  }
  if (Test-ClaudePlugin) {
    if (-not (Recorded 'claude_plugin')) { Record 'claude_plugin' 'preexisting' }
    Invoke-Quiet { claude plugin update $Plugin } | Out-Null
    Say 'skill: Claude Code plugin up to date'
  } else {
    Invoke-Checked "installing the Claude Code plugin $Plugin" { claude plugin install $Plugin }
...
function Install-CodexPlugin {
  $ErrorActionPreference = 'Continue'
  if (-not (Have 'codex')) { Say 'skill: Codex not found; skipped'; return }
  if (-not (Test-Path -LiteralPath $CodexDir)) {
    New-Item -ItemType Directory -Force -Path $CodexDir -ErrorAction Stop | Out-Null
    Record 'codex_dir' "created:$CodexDir"
  }
  $src = if ($state.Source) { $state.Source } else { "$Repo@$($state.Version)" }
  if ((Test-CodexMarketplace) -and (Recorded 'codex_marketplace') -eq 'added' -and (Recorded 'codex_marketplace_source') -ne $src) {
    Invoke-Checked 'removing the old Codex marketplace' { codex plugin marketplace remove $Marketplace }
  }
  if (Test-CodexMarketplace) {
    if (-not (Recorded 'codex_marketplace')) { Record 'codex_marketplace' 'preexisting' }
    if (-not (Invoke-Quiet { codex plugin marketplace upgrade $Marketplace })) { Warn 'could not refresh the Codex marketplace' }
  } else {
    Invoke-Checked "adding the Codex marketplace from $src" { codex plugin marketplace add $src }
    Record 'codex_marketplace' 'added'
    Record 'codex_marketplace_source' $src
  }
  if (Test-CodexPlugin) {
    if (-not (Recorded 'codex_plugin')) { Record 'codex_plugin' 'preexisting' }
  } else {
    Record 'codex_plugin' 'installed'
  }
  # `add` also refreshes an installed plugin to the marketplace's current version.
  Invoke-Checked "installing the Codex plugin $Plugin" { codex plugin add $Plugin }
...
function Invoke-Install {
  ...
  try {
    ...
    if (-not $NoPlugin) { Install-ClaudePlugin; Install-CodexPlugin }
