...
# tests/validate-suite-manifest.sh where the two must agree. Ten of the
...
# splitting the file would break that pairing.
#
...
make_fixture() {
    ...
    cat > "$WORK_DIR/nested/run-all.sh" <<EOF
...
# The two symlink cases below build a directory OUTSIDE tests/ to point at.
# Without a trap, an abort between mktemp and the matching rm leaves it behind,
# and the leak is invisible because the tests that create it are the ones that
# may legitimately skip.
OUTSIDE_DIRS=()

cleanup_outside() {
    local d
    for d in ${OUTSIDE_DIRS+"${OUTSIDE_DIRS[@]}"}; do
        [ -n "$d" ] && [ -d "$d" ] && rm -rf "$d"
    done
    OUTSIDE_DIRS=()
}
trap 'cleanup_fixture; cleanup_manifest; cleanup_outside' EXIT

# Unlike make_fixture this copies the whole tests tree, runner-guard.sh
...
manifest_exit() {
    local status=0
    ( cd "$MANIFEST_DIR" && bash validate-suite-manifest.sh >/dev/null 2>&1 ) \
        || status=$?
    echo "$status"
}
...
# is rejected outright rather than interpreted. `#` delimiter, not `|`: the
# manifest's own field separator is `|`, so a `|`-delimited expression here
# would be parsed as extra sed fields, not content.
reset_manifest_dir
sed -i 's#^Comment Rules Guard|comment-rules-guard\.sh$#&|allow_zero#' \
...
make_fixture 0 0
printf 'Nested Suite|nested/run-all.sh|allow_zero
Skill Validation|validate-skills.sh
' > "$WORK_DIR/required-suites.txt"
run_fixture
assert_exit_status "$FIXTURE_STATUS" 1 \
...
reset_manifest_dir
sed -i 's#^MANIFEST=.*#MANIFEST="$TESTS_DIR/somewhere-else.txt"#' \
    "$MANIFEST_DIR/run-all.sh"
assert_exit_status "$(manifest_exit)" 1 \
...
OUTSIDE_DIRS+=("$RUNNER_OUTSIDE")
printf '#!/usr/bin/env bash
echo "Results: 9 passed"
' > "$RUNNER_OUTSIDE/evil.sh"
MSYS=winsymlinks:nativestrict \
    ln -s "$RUNNER_OUTSIDE/evil.sh" "$WORK_DIR/escaped.sh" 2>/dev/null || true
