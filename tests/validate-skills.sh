...
# Every references/<file>.md a SKILL.md names is handed to a dispatched subagent as a
# literal path. A typo there fails silently in the worst direction: the subagent cannot
# read the file, says nothing about it, and still returns a confident clean review.
# Nothing else in this suite catches it, because the skill markdown stays well-formed.
#
# Three path shapes appear across the skills, and all three resolve against the skill
# directory: a bare `references/x.md`, the `/references/x.md` tail of an
# `<announced base>/references/x.md` instruction, and a sibling-skill
# `../<skill>/references/x.md`. One pattern covers them because the optional `../` group
# is the only part that leaves the skill directory. `../<skill>/references/x.md` is the
# supported way to name another skill's reference; an announced-base path that crosses
# skills would resolve here against the wrong directory and is reported as missing.
#
# Echoes the paths that do not resolve, space-separated, and nothing when they all do.
# `|| true` is load-bearing under `set -euo pipefail`: grep exits 1 when a skill names no
# reference at all, which is legal -- 6 of the 13 skills do -- and must not abort the run.
...
# The loop above only ever runs missing_references_for against a tree where every reference
# resolves, so on its own it proves the check exists, not that it works -- and testing-rules.md
# is explicit that a check never seen red is not yet a test. These two cases run the same
# function against a sandbox built to fail and one built to pass, which is what makes a later
# rewrite of the pattern detectable. Sandboxes live outside skills/, so the loop cannot see them.
SANDBOX=$(mktemp -d)
