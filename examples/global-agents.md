...
<!-- customize: keep only rows for skills you have installed, spelled exactly as your harness lists
     ...
     a Claude Code link install lists it bare. Add a row for each situation where you want a
     particular skill used, in the same shape: the situation, then `<plugin>:<skill>`. -->
...
with its rough scale instead of launching it. When delegating, load
`necturalabs:agent-orchestration`. Always:
...
- Every change the review row above covers gets an independent review before commit, merge or
  ...
  and the exact diff, never the reasoning that produced it.
...
- Evidence is the command and its result — quote output where it shows a failure or a result I
  ...
  was tested.
- Monitor delegated work as `necturalabs:agent-orchestration` says; never poll just to learn whether
  a subagent finished.
