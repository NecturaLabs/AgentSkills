#!/bin/sh
# Round-trip test for scripts/install-fab.sh in a throwaway HOME.
#
# Installs from this checkout (FabCLI is downloaded and checksum-verified), checks every artifact
# landed, uninstalls, and fails if anything named after necturalabs-fab or FabCLI, or any reference
# to them, is left behind. The real HOME is never touched; sign-in is skipped.
...
sh "$ROOT/scripts/install-fab.sh" --no-login --bin-dir "$BIN" --source "$ROOT"
...
sh "$ROOT/scripts/install-fab.sh" --uninstall
...
if [ "$(uname -s)-$(uname -m)" = Linux-x86_64 ]; then
  if sh "$ROOT/scripts/install-fab.sh" --no-login --bin-dir "$FOREIGN" --source "$ROOT" 2>/dev/null; then
...
sh "$ROOT/scripts/install-fab.sh" --no-login --bin-dir "$BIN" --source "$ROOT" >/dev/null
sh "$ROOT/scripts/install-fab.sh" --uninstall >/dev/null
