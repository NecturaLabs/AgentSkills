#!/bin/sh
# The Fab step of the AgentSkills install, for Linux and macOS.
#
...
#
# Installs FabCLI (pinned, checksum-verified, from its upstream releases) and necturalabs-fab (from
# AgentSkills releases) into one bin directory and offers to sign in. The fab skill itself ships in
# the necturalabs plugin, which the AgentSkills install covers. Everything this script creates is
# recorded in a manifest; --uninstall removes exactly that and nothing else, including the
# necturalabs-fab plugin and marketplace an installer from before the merge into AgentSkills added.
...
usage() {
  cat <<'USAGE'
Usage: install-fab.sh [options]

Install:
  --bin-dir DIR     Where fabcli and necturalabs-fab go (default: ~/.local/bin,
                    or $NECTURALABS_FAB_BIN_DIR).
  --version TAG     AgentSkills release to install, e.g. v5.0.0 (default: the
                    latest release, or $NECTURALABS_FAB_VERSION).
  --source DIR      Build necturalabs-fab from this AgentSkills checkout.
                    Default when the script runs from a checkout.
...
detect_checkout() {
  ...
  if [ -f "$dir/cli/fab/Cargo.toml" ] && grep -q '^name = "necturalabs-fab"' "$dir/cli/fab/Cargo.toml"; then
    SOURCE=$dir
  fi
}

# The AgentSkills release the necturalabs-fab binary comes from.
resolve_tag() {
  [ -n "$SOURCE" ] && return 0
  [ -n "$TAG" ] && return 0
  url=""
...
install_necturalabs_fab() {
  ...
  if [ -n "$SOURCE" ]; then
    ...
    (cd "$SOURCE/cli/fab" && cargo build --release --locked --bin necturalabs-fab --quiet)
    install -m 755 "$SOURCE/cli/fab/target/release/necturalabs-fab" "$BIN_DIR/necturalabs-fab"
  else
...
do_install() {
  ...
  say ""
  if [ "${VIA_GH:-0}" = 1 ]; then
    say "Done. Uninstall with: gh api -H 'Accept: application/vnd.github.raw' repos/$REPO/contents/scripts/install-fab.sh | sh -s -- --uninstall"
  else
    say "Done. Uninstall with: curl -fsSL https://raw.githubusercontent.com/$REPO/main/scripts/install-fab.sh | sh -s -- --uninstall"
  fi
}
